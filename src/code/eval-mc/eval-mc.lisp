;;;; A minimally compiling EVAL (main entry point)
...
(defun prepare-progn (forms)
  (labels ((flatten-progns (forms)
             ;; Optimization: Hoist up bodies of PROGNs that are
             ;; nested directly in PROGN-like blocks.
             (flet ((nested-progn-p (form)
                      (and (listp form) (eql (car form) 'progn))))
               (if (member-if #'nested-progn-p forms)
                   (loop for form in forms
                         if (nested-progn-p form)
                         append (or (flatten-progns (cdr form))
                                    '(nil))
                         else
                         collect form)
                   forms))))
    (let* ((flattened (flatten-progns forms))
           (body* (mapcar (lambda (form) (prepare-form form))
                          flattened)))
      (cond ((null body*)
             (prepare-nil))
            ((endp (rest body*))
